...
//		server-side validation
		if(fname == null || fname.equals("")) {
			response.sendRedirect("AddUser.jsp?regFailed=true");
		}
            ...
            response.sendRedirect("AddUser.jsp?emailExists=true");
        }		
		else if(phone == null || phone.equals("") || phone.length() != 10) {
			response.sendRedirect("AddUser.jsp?invalidPhone=true");
		}
		else if(password == null || password.equals("")) {
			response.sendRedirect("AddUser.jsp?regFailed=true");
		}
		else if(pwdMatcher.matches() == false) {
            response.sendRedirect("AddUser.jsp?invalidPassword=true");
		}
		else if(!password.equals(repass)) {
			response.sendRedirect("AddUser.jsp?passNoMatch=true");
		}		
		else {	
			try {
